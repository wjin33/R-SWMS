...
!> \param nel = nex*ney (number of elements per horizontal element-layer)
  ...
  USE ParamData, ONLY: ldirect,lvtk,lOutPartrace,lChem,lretry,last_out,&
       lSalinity,maxmat,maxIrrig,maxbdr,mxBcCh,lPartUp,lobstacle
...
77 READ(10,*)
  ...
  !> #### reads obstacle.in for obstacles informations ####
  IF (lobstacle) then
    OPEN(Unit=10,FILE='in/obstacles.in',STATUS='OLD',ERR=60)
    READ(10,*)
    READ(10,*)
    READ(10,*)
    READ(10,*) nobstacles
    ...
    READ(10,*)
    Do i=1,nobstacles
      READ (10,*) xminObs(i), xmaxObs(i), yminObs(i), ymaxObs(i), zminObs(i), zmaxObs(i)
    END DO
    READ(10,*)
    READ(10,*)
    Do i=1,nElm
...
SUBROUTINE DouIn
  ...
  USE RootData, ONLY: sign_in,PH_crit,delta_h,lSign,lSign_new,lSign_inst,size_buff,vol_root,&
  ...
  SELECT CASE(stresfun)
     ...
     CASE(5) !> + case5: Hydraulic and hormonal regulation of stomata; Tardieu et al. (1993) function; NO hormone transport
        ...
        ELSE
           vol_buff = segsur(1)*segsur(1)/seglen(1)/4/pi
        END IF
     CASE(6) ! Tardieu and Davies Updated
	lSign_new=.TRUE.
...
SUBROUTINE ClimateIn
   ...
   READ (10,*,ERR=20) (dummy,time_climate(i),PPFD(i),Temperature(i),VPD(i),i=1,nclimaticdata)
   !time_climate=t_initial+time_climate/24.
   CLOSE (10)
   print*,'... Reading Climate.in ...'
   RETURN
10 STOP 'Climate.in not found, program terminated'
20 STOP 'Problem in reading Climate.in'
...
SUBROUTINE ProcessClimate
   ...
   INTEGER:: count_day=1,i,ndays,j,j_init
   REAL :: sum_daily_PPFD
...
SUBROUTINE TardieuIn
   ...
   RETURN
10 STOP 'Tardieu.in not found, program terminated'
20 STOP 'Problem in reading Tardieu.in'
...
81      linlim=MIN(ifive+5,nestbl(igrow))
  ...
  IF (lSomma_growth) THEN
     ...
     END DO
     WRITE (*,'('' ... 1st passedmake  <Root.in> ...'')')
     naxtot=naxes
     ifg(1:naxtot)=1 !initial axes always belong to axis group #1
     DO i=1,naxemg
...
SUBROUTINE PlntIn(t)
  ...
  USE PlntData
  USE RootData,ONLY : lDou,lCou,lFed,lSomma_growth,lno_RWU,lCalloc,lno_Archi,ltemp,ltoxi,lSign_new,timeobs

...
SUBROUTINE Solstr(h)
  ...
  USE RootData, ONLY : l_overburden
  USE DomData
  IMPLICIT NONE
  INTEGER(ap) :: i,m
  REAL(sp), INTENT(in) ::  h(nPt)
  REAL(sp) :: sat,dummy,por,voidratio,delta1,stress
  ! s = soil strength
      ...
      ELSE
        ...
        IF (sat .LE. 0._sp) sat=0.001_sp
          ...
          por=1-par(11,M)/2.65 !estimated porosity
			    IF (por .EQ. 1._sp) por=0.99_sp
            voidratio=por/(1-por)
            !Print*, 'zgrid(nPt)', ABS(zGrid(i))
            stress=9.81*par(11,M)*ABS(zGrid(i))*0.01 !overburden
            !Print*, 'sigma', stress
			    IF (sat .LT. 0.5) sat=0.5_sp	
            s(i)=9.81*par(11,M)*((3.560-voidratio)**2/(1+voidratio)*((stress**2.1931)-(h(i)*0.1*sat))**0.1846)**2*0.001 !MPa
            !Print*, 'soil penetration resistance', s(i)
		    ELSE !approach by Whalley et al (2016)
          dummy=0.35*LOG10(abs(h(i))*0.1*sat)+0.93*par(11,M)+1.26 !kPa
          s(i)=10**dummy*0.001 !MPa
          !print*,dummy,s(i),h(i),sat,par(11,M)
		    ENDIF
      ENDIF

...
MODULE ParamData
  USE typedef
  IMPLICIT NONE
  INTEGER(ap),PARAMETER :: maxnod=662661,mxpnts=100,maxplant=2,maxobstacle=100
  ...
  LOGICAL :: lOrt, lSalinity=.true.,lPartUp=.FALSE., lobstacle=.FALSE.
...
MODULE RootData
  ...
  INTEGER(ap) :: naxes,norder,ntimeobs
  ...
  REAL(sp), ALLOCATABLE, DIMENSION(:) :: Rho,tPast,AQPh,AQPv,timeobs,Krs_mat,Kcomp_mat
  REAL(sp), ALLOCATABLE, DIMENSION(:,:) :: sinkredDPast,distseg2soil
  REAL(dp) :: mhorm(maxrec,maxplant),concol,mcol,msign_notrans,csign_notrans,res_t,delta_h,mcol_i=0._dp
  ...
  LOGICAL :: lCalloc=.FALSE.,ltemp=.FALSE.,lSign=.FALSE.,lSign_new=.FALSE.,lSign_inst=.FALSE.,ltoxi=.FALSE.,lSinkCube=.FALSE.
...
MODULE EnviData
  USE typedef
  IMPLICIT NONE
  REAL(sp), ALLOCATABLE, DIMENSION(:) :: time_climate,PPFD,VPD,Temperature,PPFDcum,Rn
  REAL(sp), ALLOCATABLE, DIMENSION(:) :: s_f, rho_f, gamma_f, conversion_f,lambda_f
  REAL(sp), ALLOCATABLE, DIMENSION(:) :: hours,days,ampli,psixylmax,psixylmin
  REAL(sp), ALLOCATABLE, DIMENSION(:,:) :: datad
  REAL(sp) :: Cp
  REAL(sp) :: t_initial=0.
  INTEGER(ap) :: nclimaticdata
  REAL(sp) :: PPFD_t,VPD_t,Temperature_t,Rn_t
  REAL(sp) :: s_t, rho_t, gamma_t, conversion_t,lambda_t,gs_t,Jxc_t
  REAL(sp) :: Hleaf,Hbundle,Hcell,Vcel

  CONTAINS

	REAL(sp) FUNCTION gs_fun(ABA,Psi,gsmin,gsalpha,gsbeta,gsdelta)
	REAL(sp),INTENT(IN):: ABA,Psi,gsmin,gsalpha,gsbeta,gsdelta
	...
	REAL(sp) FUNCTION PM_fun(gs,Sshaded,s,Rn,rho,Cp,ga,VPD,lambda,gamma,conversion)
	REAL(sp),INTENT(IN):: gs,Sshaded,s,Rn,rho,Cp,ga,VPD,lambda,gamma,conversion
	PM_fun = Sshaded*(s*Rn + rho *Cp*ga*VPD)/(lambda*(s+gamma*(1+ga*conversion/gs)))
	RETURN
	END FUNCTION PM_fun

	REAL(sp) FUNCTION PHcollar_fun(PHeq,Krs,Tact)
	REAL(sp),INTENT(IN)::PHeq
	REAL(dp),INTENT(IN):: Tact,Krs
	PHcollar_fun=PHeq-Tact/Krs
	!print*,PHcollar_fun
	END FUNCTION PHcollar_fun

	REAL(sp) FUNCTION ABA_collar_fun(Hseq,Tact,ABAconstit,ABAa,ABAb)
        REAL(sp),INTENT(IN)::Hseq,ABAconstit,ABAa,ABAb
...
MODULE TardieuData
  USE typedef
  IMPLICIT NONE
  REAL(sp) :: gsmin,gsalpha,gsbeta,gsdelta
  REAL(sp) :: ampliinit,TaucircadGr,TaucircadGxl,TaucircadGstem,TaucircadGc
  REAL(sp) :: TautranspiGr,TautranspiGxl,TautranspiGstem,TautranspiGc
  REAL(sp) :: TauABAGr,TauABAGxl,TauABAGstem,TauABAGc
  REAL(sp) :: Vres,Vsat,ncap,alphacap
  REAL(sp) :: Grmin,Grmax,Gxlmin,Gxlmax,Gstemmin,Gstemmax,Gcmin,Gcmax,Gxl0,Gstem0,Gc0
  REAL(sp) :: Gc,Gxl,Gstem
  REAL(sp) :: ABAconstit,ABAa,ABAb
  REAL(sp) :: S,shading,ga,Sshaded
  Real(sp) :: ABA_collar=0.,Krs0,Kcomp0
  REAL(sp), ALLOCATABLE, DIMENSION(:,:) :: Global_transpi_and_Psi
  REAL(sp) :: Krs_transpi,Krs_circad
...
MODULE GridData
  ...
  REAL(sp), ALLOCATABLE,DIMENSION(:) :: Wn,sink,csink,sinkOld,sink_cube,sink_cubeOld,csink_cube,betac_cube,betac_cube2,cmass_cube
...
MODULE DoussanMat
  ...
  INTEGER(ap), ALLOCATABLE, DIMENSION(:) :: nBC_irecn,nBC_iprvn,no_voxels,segPerCube !ija,sa,
...
!> Data for soil domain
 MODULE DomData
   USE TypeDef
   USE ParamData, ONLY: maxobstacle, maxelm
   IMPLICIT NONE
   REAL(sp):: xmin,xmax,ymin,ymax,zmin,zmax
   INTEGER(ap) ::nobstacles
   REAL(sp):: xminObs(maxobstacle),xmaxObs(maxobstacle),yminObs(maxobstacle),ymaxObs(maxobstacle)
   REAL(sp):: zminObs(maxobstacle),zmaxObs(maxobstacle)
   INTEGER(ap) ::EleTran(maxelm)
 END MODULE DomData
 !********************************************************************

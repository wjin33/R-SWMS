...
SUBROUTINE ROOT(t,dt)
  USE typedef
  USE ParamData, ONLY: pi,lobstacle
  USE RootData
...
SUBROUTINE Neighb(x,y,z,corner,imin)
  ...
  USE GridData
  USE DomData
  USE ParamData, ONLY: lobstacle
  IMPLICIT NONE

  REAL(sp), INTENT(in) :: x,y,z
  REAL(sp):: delta1
  INTEGER(ap):: ixmin,ixymin,i
  ...
  ! IF obstacles in simulated, we need redirect the Element ID
  IF (lobstacle) THEN
  Do i = 1, nElm
    IF (EleTran(i)==imin) THEN
      imin=i;
    END IF
    EXIT
  END DO
...
SUBROUTINE Length(v,tgrow,drivex,drivey,drivez,condmatrix,newlen,unimplen,corner)
  ...
  ! calculate length of new segment, taking location into account, approach different for soil strength/conductance approach:
   ...
   ELSE
    ...
    !print*,lMacro
      ...
      !print*,imps(corner(1)),imps(corner(7))
		ENDIF
   ENDIF
  ...
  ELSE
     cncimp = 1._sp
  ENDIF
  !print*,'cncimp*strimp*temimp',cncimp,strimp,temimp,v,tgrow
...
!> \param smax = maximum soil strength of one grid cell node
  ...
  !> \param condmatrix = conductance tensor
   ...
   DO i=1,8
	...
	IF (condu(corner(i)).GE.1.E1_sp) THEN
...
1 iret = irecsg(igrow) ! tip node
  ...
  IF ( wall.EQ.'xmin' .OR. wall.EQ.'xmax' ) dx=0.0_sp
  IF ( wall.EQ.'ymin' .OR. wall.EQ.'ymax' ) dy=0.0_sp
  IF ( wall.EQ.'zmin' .OR. wall.EQ.'zmax' ) dz=0.0_sp
...
SUBROUTINE ObstacleLim(igrow)
  ...
  INTEGER(ap), INTENT(in) :: igrow
  INTEGER(ap) :: irec,iret,imin,i
  REAL(sp):: dx,dy,dz,newtime
  REAL(dp):: factor,fractn,del,length,newlen,mass,newmass,ratio,xvalue,yvalue,zvalue
  REAL(dp):: centerx, centery, centerz,centerx_seg,centery_seg,centerz_seg,temp
  CHARACTER wall*4
  LOGICAL :: Touch
  !> \param igrow number of growing tip
  ...
  centerx_seg=(xs(iret) + xs(irec))/2
  centery_seg=(ys(iret) + ys(irec))/2
  centerz_seg=(zs(iret) + zs(irec))/2


  ! domain walls are the growth boundarys
  DO i=1,nobstacles
      IF ( xs(iret).GT.xminObs(i) .AND. xs(iret).LT.xmaxObs(i) ) THEN
        IF ( ys(iret).GT.yminObs(i) .AND. ys(iret).LT.ymaxObs(i) ) THEN
          IF ( zs(iret).GT.zminObs(i) .AND. zs(iret).LT.zmaxObs(i) ) THEN
            Touch = .true.
            Exit
          ENDIF
        ENDIF
      ...
      IF ( (xs(irec)-xminObs(i))*(xs(iret)-xminObs(i))<=0 .OR. (xs(irec)-xmaxObs(i))*(xs(iret)-xmaxObs(i))<=0  ) THEN
        IF ( (ys(irec)-yminObs(i))*(ys(iret)-yminObs(i))<=0) THEN
          temp= (zs(irec)+zs(iret))/2
          IF (temp.GT.zminObs(i) .AND. temp.LT.zmaxObs(i)) THEN
            Touch = .true.
            Exit
          ENDIF
        ELSEIF ( (ys(irec)-ymaxObs(i))*(ys(iret)-ymaxObs(i))<=0 ) THEN
          temp= (zs(irec)+zs(iret))/2
          IF (temp.GT.zminObs(i) .AND. temp.LT.zmaxObs(i)) THEN
            Touch = .true.
            Exit
          ENDIF
        ELSEIF ( (zs(irec)-zmaxObs(i))*(zs(iret)-zmaxObs(i))<=0 ) THEN
          temp= (ys(irec)+ys(iret))/2
          IF (temp .GT.yminObs(i) .AND. temp.LT.ymaxObs(i)) THEN
            Touch = .true.
            Exit
          ENDIF
        ELSEIF ( (zs(irec)-zminObs(i))*(zs(iret)-zminObs(i))<=0 ) THEN
          temp= (ys(irec)+ys(iret))/2
          IF (temp .GT.yminObs(i) .AND. temp.LT.ymaxObs(i)) THEN
            Touch = .true.
            Exit
          ENDIF
        ENDIF
      ELSEIF ( (ys(irec)-yminObs(i))*(ys(iret)-yminObs(i))<=0 .OR. (ys(irec)-ymaxObs(i))*(ys(iret)-ymaxObs(i))<=0  ) THEN
        IF ( (xs(irec)-xminObs(i))*(xs(iret)-xminObs(i))<=0) THEN
          temp= (zs(irec)+zs(iret))/2
          IF (temp.GT.zminObs(i) .AND. temp.LT.zmaxObs(i)) THEN
            Touch = .true.
            Exit
          ENDIF
        ELSEIF ( (xs(irec)-xmaxObs(i))*(xs(iret)-xmaxObs(i))<=0 ) THEN
          temp= (zs(irec)+zs(iret))/2
          IF (temp.GT.zminObs(i) .AND. temp.LT.zmaxObs(i)) THEN
            Touch = .true.
            Exit
          ENDIF
        ELSEIF ( (zs(irec)-zmaxObs(i))*(zs(iret)-zmaxObs(i))<=0 ) THEN
          temp= (xs(irec)+xs(iret))/2
          IF (temp .GT.xminObs(i) .AND. temp.LT.xmaxObs(i)) THEN
            Touch = .true.
            Exit
          ENDIF
        ELSEIF ( (zs(irec)-zminObs(i))*(zs(iret)-zminObs(i))<=0 ) THEN
          ...
          ENDIF
        ENDIF
      ELSEIF ( (zs(irec)-zminObs(i))*(zs(iret)-zminObs(i))<=0 .OR. (zs(irec)-zmaxObs(i))*(zs(iret)-zmaxObs(i))<=0  ) THEN
        IF ( (xs(irec)-xminObs(i))*(xs(iret)-xminObs(i))<=0) THEN
          temp= (ys(irec)+ys(iret))/2
          IF (temp.GT.yminObs(i) .AND. temp.LT.ymaxObs(i)) THEN
            Touch = .true.
        ...
        ELSEIF ( (ys(irec)-ymaxObs(i))*(ys(iret)-ymaxObs(i))<=0 ) THEN
          ...
          IF (temp .GT.xminObs(i) .AND. temp.LT.xmaxObs(i)) THEN
            ...
            Exit
          ENDIF
        ELSEIF ( (ys(irec)-yminObs(i))*(ys(iret)-yminObs(i))<=0 ) THEN
          temp= (xs(irec)+xs(iret))/2
          IF (temp .GT.xminObs(i) .AND. temp.LT.xmaxObs(i)) THEN
            Touch = .true.
            Exit
  ...
  IF(.NOT.Touch) THEN
    !WRITE (*,'('' Not Touch Yet ...'')')
    RETURN !The tip is not inside the obstacle
  ELSE
    centerx=(xminObs(i) + xmaxObs(i))/2
    centery=(yminObs(i) + ymaxObs(i))/2
    centerz=(zminObs(i) + zmaxObs(i))/2
    WRITE (*,'(''....Touching ...'')')
    WRITE (*,"(3F10.6)")xs(irec), ys(irec), zs(irec)
    WRITE (*,"(3F10.6)")centerx, centery, centerz
    IF(xs(irec) <= centerx .AND. ys(irec) <= centery .AND. zs(irec)<= centerz )THEN

      ratio=abs((zminObs(i)-zs(irec))/(zs(iret)-zs(irec)))
      xvalue = xs(irec)+ratio*( xs(iret) - xs(irec) )
      yvalue = ys(irec)+ratio*( ys(iret) - ys(irec) )
      IF ( xvalue >= xminObs(i) .AND. xvalue <= xmaxObs(i) ) THEN
        IF ( yvalue >= yminObs(i) .AND. yvalue <= ymaxObs(i) ) THEN
          del = ratio*seglen(iret)
          wall   = 'zmin'
          GOTO 2
        ENDIF
      ENDIF
      ratio=abs((xminObs(i)-xs(irec))/(xs(iret)-xs(irec)));
      zvalue = zs(irec)+ratio*( zs(iret) - zs(irec) );
      yvalue = ys(irec)+ratio*( ys(iret) - ys(irec) );
      IF ( zvalue >= zminObs(i) .AND. zvalue <= zmaxObs(i) ) THEN
        IF ( yvalue >= yminObs(i) .AND. yvalue <= ymaxObs(i) ) THEN
          del = ratio*seglen(iret)
          wall   = 'xmin'
          GOTO 2
        ENDIF
      ENDIF
      ratio=abs((yminObs(i)-ys(irec))/(ys(iret)-ys(irec)));
      zvalue = zs(irec)+ratio*( zs(iret) - zs(irec) );
      xvalue = xs(irec)+ratio*( xs(iret) - xs(irec) );
      IF ( zvalue >= zminObs(i) .AND. zvalue <= zmaxObs(i) ) THEN
        IF ( xvalue >= xminObs(i) .AND. xvalue <= xmaxObs(i) ) THEN
    ...
    ELSEIF(xs(irec) <= centerx .AND. ys(irec) <= centery .AND. zs(irec) >= centerz)THEN

      ratio=abs((zmaxObs(i)-zs(irec))/(zs(iret)-zs(irec)))
      xvalue = xs(irec)+ratio*( xs(iret) - xs(irec) )
      yvalue = ys(irec)+ratio*( ys(iret) - ys(irec) )
      IF ( xvalue >= xminObs(i) .AND. xvalue <= xmaxObs(i) ) THEN
        IF ( yvalue >= yminObs(i) .AND. yvalue <= ymaxObs(i) ) THEN
    ...
    ELSEIF(xs(irec) <= centerx .AND. ys(irec) >= centery .AND. zs(irec) <= centerz)THEN
      ...
      IF ( zvalue >= zminObs(i) .AND. zvalue <= zmaxObs(i) ) THEN
        ...
        ENDIF
      ENDIF
      ratio=abs((ymaxObs(i)-ys(irec))/(ys(iret)-ys(irec)));
      zvalue = zs(irec)+ratio*( zs(iret) - zs(irec) );
      xvalue = xs(irec)+ratio*( xs(iret) - xs(irec) );
      IF ( zvalue >= zminObs(i) .AND. zvalue <= zmaxObs(i) ) THEN
        IF ( xvalue >= xminObs(i) .AND. xvalue <= xmaxObs(i) ) THEN
    ...
    ELSEIF(xs(irec) >= centerx .AND. ys(irec) <= centery .AND. zs(irec) <= centerz )THEN
      ...
      IF ( xvalue >= xminObs(i) .AND. xvalue <= xmaxObs(i) ) THEN
        ...
        ENDIF
      ENDIF
      ratio=abs((xmaxObs(i)-xs(irec))/(xs(iret)-xs(irec)));
      zvalue = zs(irec)+ratio*( zs(iret) - zs(irec) );
      yvalue = ys(irec)+ratio*( ys(iret) - ys(irec) );
      IF ( zvalue >= zminObs(i) .AND. zvalue <= zmaxObs(i) ) THEN
        IF ( yvalue >= yminObs(i) .AND. yvalue <= ymaxObs(i) ) THEN
  ...
  ! if we get to here , we know we have the tip inside the obstacle and need
  ! new heading components:
2 dx = xs(iret)-xs(irec)
  dy = ys(iret)-ys(irec)
  dz = zs(iret)-zs(irec)
  WRITE (*,'('' Touched...'')')
  WRITE(*,"(A)")  wall
  ...
  IF (del.GT.1.E-06_sp) THEN
     ...
     seglen(iret) = del
     ! pull back tip to start part with new heading:
     WRITE (*,"(3F10.6)")xs(iret), ys(iret), zs(iret)
     xs(iret) = xs(irec)+fractn*(xs(iret)-xs(irec))
     ys(iret) = ys(irec)+fractn*(ys(iret)-ys(irec))
     zs(iret) = zs(irec)+fractn*(zs(iret)-zs(irec))
     
     ! to prevent numerical problems, pull back exactly to wall
     IF (wall.EQ.'xmin') xs(iret) = xminObs(i)
     IF (wall.EQ.'xmax') xs(iret) = xmaxObs(i)
     IF (wall.EQ.'ymin') ys(iret) = yminObs(i)
     IF (wall.EQ.'ymax') ys(iret) = ymaxObs(i)
     IF (wall.EQ.'zmin') zs(iret) = zminObs(i)
     IF (wall.EQ.'zmax') zs(iret) = zmaxObs(i)
     ...
     newlen=length - seglen(iret)
     newmass=mass - segmas(iret)
     WRITE(*,"(F10.6)")  newlen
     IF (newlen.GT.1.E-06_dp) THEN
        newtime = timorg(iret)+fractn*dtRoot !convert to sp!
        !toosml(nrec)=.FALSE.
          WRITE(*,"(3F10.6)")  dx,dy,dz
        CALL Grow(igrow,dx,dy,dz,newlen,newmass,newtime)
     ENDIF
  ELSE
     ! don't split, change heading only --
     ! calculate the actual length of components dx,dy,dz:
...
10 ifrom = ifrom+1
  ...
  ELSE IF (ito.GT.0) THEN
     ...
     !print*,'Before update',xg(ifrom),yg(ifrom),zg(ifrom)
     xg(ito)      = xg(ifrom)
     yg(ito)      = yg(ifrom)
     zg(ito)      = zg(ifrom)

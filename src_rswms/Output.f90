...
SUBROUTINE WriteLog(t)
  ...
  DO ipl=1,nplant
     ...
     ELSEIF (lDou .AND. .NOT.lCalloc) THEN
        ...
        elseif(lPartUp) then
           WRITE (10,'(4(1pE11.4,1X),I6,1X,6(1pE11.4,1X))')&
                t,Tpot,Tact(ipl),sAvg,count_nodes,PHr(1,ipl),mcol,concol
        else
     ...
     ELSEIF (lCou) THEN
	IF (lSign_new) THEN
           WRITE (10,'(4(1pE11.4,1X),I6,1X,5(1pE11.4,1X))')&
                t,Tpot,Tact(ipl),sAvg,count_nodes,PHcollar,Hseq,Krs,gs_t,conversion_t
        ELSE
          WRITE (10,'(3(1pE11.4,1X),I2,1X,1pE11.4,1X,1pE11.4,1X,1pE11.4,1X,1pE11.4)')&
               t,Tpot,Tact(ipl),0,PHcollar,DOT_PRODUCT(HElm(1:nElm),SSF),Krs,Kcomp
        ENDIF
     ENDIF
  END DO
  RETURN
...
SUBROUTINE OutDou(t,kout)
  ...
  DO ipl=1,nplant
    ...
    WRITE (8,'(I6,3(1X,1pE10.3),I5,1X,I6,1X,E9.3,1X,E9.3,1X,E13.5,1X,E13.5,E13.5,8(1X,E13.5))') &
     ...
     END DO
     CLOSE(8)
90   FORMAT(' segID          x          y         z     br#    prev     Lr     Khr      Phinter     PHxylem     radialRootFlow     axialRootFlow      Qi        Qd       Q_bc       radius       veloRoot           segconc ')
	Else
DO irec=1,nrec
...
SUBROUTINE OutVTK(kOut)
  ...
  USE RootData, ONLY: lSomma_growth,lcou
  USE StrData, ONLY: s
  USE RhizoData
  IMPLICIT NONE
  ...
  IF (continu) THEN
    ...
    WRITE (8,'(/A5,1X,I7,1X,I8)')'CELLS', nElm, 9*nElm
    do iE=1,nElm
        WRITE (8,'((1X,I6))',advance="no") 8
        do global=1,8
            WRITE (8,'((1X,I6))',advance="no") elmnod(global, iE)-1
        end do
        WRITE (8,*)''
    enddo
  
    WRITE (8,'(/A10,1X,I7)')'CELL_TYPES', nElm
    do iE=1,nElm
        WRITE (8,'(8(1X,I2))',advance="no") 12
    enddo
...
!!$           END DO
  ...
  ELSE
    ...
    WRITE (8,'(/A5,1X,I7,1X,I8)')'CELLS', nElm, nElm*9
    do iE=1,nElm
        WRITE (8,'((1X,I6))',advance="no") 8
        do global=1,8
            WRITE (8,'((1X,I6))',advance="no") elmnod(global, iE)-1

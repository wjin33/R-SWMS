...
222 CONTINUE
  ...
  IF(lPartUp) THEN
     ...
     !print*,'hier mcubetot', mcubetot+sum(m_upt)
  END IF
...
92      FORMAT('  Time       Tpot       Tact  soil_lim_nodes    H_collar      H_seq	Krs	Kcomp')
93      FORMAT('  Time       Tpot       Tact   sAvg    soil_lim_nodes    H_collar')
94      FORMAT('  Time       Tpot       Tact   sAvg     soil_lim_nodes    H_collar   mcol   segconc')
95	FORMAT('  Time       Tpot       Tact   sAvg     soil_lim_nodes    H_collar   H_seq	Krs	gs_t')
        CLOSE (10)
     ENDDO
  ELSE
  ...
  IF (lSign_new) THEN
	...
	OPEN (UNIT=10,FILE=file2,STATUS='UNKNOWN')
        Write(10,96)
96	FORMAT('Time		Krs	Krs_circad	ampli	Krstranspi 	ABA	PHcollar	Hleaf	Hbundle		Hcell	Jxc	Vcel')
	CLOSE (10)
  ENDIF

      ...
      IF(nMat.GT.1) THEN
         DO iMat=1,nMat 
            ...
            ELSEIF(par(11,iMat).LE.1E-5_sp)  THEN  !macropore
               ...
               IF (l_elmMacro(imin) .AND. n_neigh(imin).LT.1)  Lr(irecn) = 0._dp
            END IF
         ...
         Branchloop: DO ibr=1,nbr
            ...
            DO WHILE (run)
               ...
               IF (irecn .EQ. 0) THEN
                  ...
                  run=.FALSE.
               ENDIF
               !> get total flow; which is addition of all flows at each large branch connecting to the seed		Quid des branches lat\E9rales ? 
               IF (iprvn .EQ. 0) THEN
...
SUBROUTINE calc_actual_climate(t)
   ...
   REAL(sp), INTENT(in)::t
   REAL(sp) :: temp_PPFD,temp_VPD,temp_temperature
   REAL(sp) :: temp_s,temp_rho,temp_gamma,temp_conversion,temp_lambda
...
SUBROUTINE interp_linear (data_num, t_data, p_data, &
  ...
  integer ( kind = 4 ) left
  real ( kind = 4 ) p_data(data_num)
  real ( kind = 4 ) p_interp
  integer ( kind = 4 ) right
  real ( kind = 4 ) t
  real ( kind = 4 ) t_data(data_num)
  real ( kind = 4 ) t_interp
...
SUBROUTINE update_Krs(t)
  ...
  IMPLICIT NONE
  REAL(sp), INTENT(in)::t
  REAL(sp) :: sum_dt,sum_Tact,hourphoto,temp_day,Jminus40=0.  
  REAL(sp) :: Gxl_transpi,Gxl_circad,Gc_transpi,Gc_circad,Gstem_transpi,Gstem_circad
  REAL(sp) :: factor_ABA
  INTEGER :: i,current_day
...
SUBROUTINE find_stomatal_aperture(Hseq,Krs,gs_loc)    
   ...
   REAL(sp), INTENT(in)::Hseq
   REAL(sp),INTENT(inout)::gs_loc
   REAL(sp)::g1,g2,difcurrent,difinit,ABA_loc,Hcollar_loc,Hbundle_loc
   REAL(sp):: J_loc
...
SUBROUTINE capacitance
  ...
  ELSE
	...
	Hbundle = PHcollar-(Tact(1)+Jxc_t)/Gstem - (Tact(1)+Jxc_t)/Gxl
        Vcel = max(0.,Vcel + Jxc_t*dt_current)

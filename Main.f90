PROGRAM RSWMS_MAIN
  ...
  USE tmctrl, ONLY: tmax,kout,tout
...
#ifdef WITH_PARTRACE
  INTEGER(ap):: i, iNodes
  REAL(dp) :: timeP,dtP,dummy_soil,dummy_root
...
Timeloop:  DO WHILE (.NOT.(ABS(t-tMax).LE.0.001_dp*dt))
     ...
     CASE(4) !passiv uptake, incl. diffusive uptake
        sinkE = fact
     CASE DEFAULT
        STOP 'Sorry, uptakeorder in Input_Partrace_etc. has to be set to a value between 1 and 4. Program terminated.'
     END SELECT
